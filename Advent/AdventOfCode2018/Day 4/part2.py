...
for guard in time_dictionary:
    ...
    freq = sorted(time_dictionary[guard].items(), key = lambda x: x[1])
    minute, freq = freq[-1]

    if freq > frequency[2]:
        frequency = (guard, minute, freq)

print(frequency)                       # guard number, minute, frequency
print(int(frequency[0]) * frequency[1])