...
def show_pokemon_by_name(
    ...
    with open(f"{PROGRAM_DIR}/pokemon.json") as file:
        ...
        # always check if alternate forms
        for pokemon in pokemon_json:
            if pokemon["name"] == name:
                forms = pokemon["forms"]
                alternate_forms = [f for f in forms if f != "regular"]
                alternate_forms.append("") # add regular form back in

        # if alternate forms, select a random form
        if alternate_forms:
            form = random.choice(alternate_forms)
            if form:
                name += f"-{form}"
